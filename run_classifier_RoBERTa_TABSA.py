...
def convert_examples_to_features(examples, label_list, max_seq_length, bpe, vocab):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        tokens_a = str(bpe.encode(example.text_a)).split()

        tokens_b = []
        if example.text_b:
            tokens_b = str(bpe.encode(example.text_b)).split()

        if tokens_b:
            # Modifies `tokens_a` and `tokens_b` in place so that the total
            # length is less than the specified length.
            # Account for <s>,  </s></> </s> with "- 4"
        ...
        # since the </s> token unambigiously separates the sequences, but it makes
        ...
        input_ids = vocab.encode_line(tokens, append_eos=False, add_if_not_exist=False).long().tolist()
